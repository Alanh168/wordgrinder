...
function Cmd.BattleSelectUI()
	...
	local function drawQueueSlot(x, y, w, h, slotIndex, monster)
		...
		local infoLines = 2
		local spriteRows = nil
		local spriteDrawY = innerY
		local spriteH = max(1, innerH - infoLines)
		if spriteUtils and spriteH >= 1 and monster.sprite_ref and monster.sprite_ref ~= "" then
			local sprite = nil
			local palette = nil
			local resampleMode = spriteUtils.RESAMPLE_NEAREST or "nearest"
			if type(spriteUtils.getSpriteByKeyMultiRes) == "function" then
				sprite, palette = spriteUtils.getSpriteByKeyMultiRes(
					monster.sprite_ref,
					"_md",
					innerW,
					spriteH,
					...
					resampleMode)
			elseif type(spriteUtils.getSpriteByKey) == "function" then
				sprite, palette = spriteUtils.getSpriteByKey(
					monster.sprite_ref,
					innerW,
					spriteH,
					BATTLE_SPRITE_PIXEL_WIDTH,
			...
			if sprite and palette and type(spriteUtils.getSpriteSize) == "function"
				and type(spriteUtils.renderSpriteRow) == "function" then
				spriteRows = sprite
				local availableSpriteH = max(1, innerH - infoLines)
				local spriteW = spriteUtils.getSpriteSize(sprite, BATTLE_SPRITE_PIXEL_WIDTH)
				local drawX = innerX + int((innerW - spriteW) / 2)
				local drawY = innerY + max(0, availableSpriteH - #sprite)
				spriteDrawY = drawY
				for row = 1, #sprite do
					spriteUtils.renderSpriteRow(drawX, drawY + row - 1, sprite[row], palette, BATTLE_SPRITE_PIXEL_WIDTH)
				end
			end
		end
		...
		local infoY
		if spriteRows then
			infoY = min(innerY + innerH - 1, spriteDrawY + #spriteRows)
		else
			infoY = max(innerY, innerY + int((innerH - infoLines) / 2))
		end
		SetNormal()
	...
	local function drawScreen()
		...
		for i = 1, queueSlots do
			...
			drawQueueSlot(x, slotY, slotInnerW, slotInnerH, i, queueSelection[i])
		end
		SetNormal()
